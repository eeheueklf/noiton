...
export default async function Home() {
  const supabase = await createClient();
    ...
    const [popularRes, workRes] = await Promise.all([
    supabase
      .from("templates")
  ...
  if (popularRes.error || workRes.error) {
    console.error("데이터 로드 실패:", popularRes.error?.message || workRes.error?.message);
  }
  ...
  <section className="max-w-[1200px] mx-auto px-6 py-12">
    ...
    <div className="flex items-center justify-between mb-8">
      <div className="flex items-center gap-4">
        <h2 className="text-[20px] font-bold tracking-tight">업무 관련 템플릿</h2>
        <span className="hidden sm:block text-[13px] text-gray-400 font-medium">ㅇㅅㅇ</span>
      </div>
      <Link 
        href="/templates" 
        className="group flex items-center text-[14px] font-bold text-[#E1C198] hover:text-[#866837] transition-colors gap-1"
