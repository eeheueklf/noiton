    ...
    <div className="group cursor-pointer">        
      ...
      <div className="flex items-start justify-between px-1">
        <div className="flex flex-col">
          <h3 className="text-[15px] font-bold leading-tight group-hover:underline">
          {keyword ? <HighlightText text={template.title} keyword={keyword} />:template.title}
          </h3>
          {showDate ? <p className="text-[13px] text-gray-500 mt-1">{formatRelativeDate(template.created_at)}</p> : 
          <p className="text-[13px] text-gray-500 mt-1">{template.creator.name || "Unknown Creator"}</p>}
        </div>
        {showStats && (<div className="flex items-center space-x-1 text-gray-400">
